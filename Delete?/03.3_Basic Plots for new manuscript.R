...
# quick for email Jan 28th
	ENMs <- read.csv(file="C:/Users/DW/Desktop/transplant_analysis/Robjects/final_site_points_snapped_to_streams/test_with_vals2.csv")
	ENMs <- ENMs[1:9, ]
	...
	binder <- merge(MainLams, ENMs, by.x="V1", by.y="ID2", all.x=TRUE, all.y=TRUE)
	par(mfrow=c(2,3))
	binder <- binder[which(binder$Latitude > 43),]
	binder$myCol <- ifelse(binder$Latitude > 43.90, "blue", "red")	
	plot(binder$Latitude, binder$RFavg, xlab="Latitude", ylab="Suitability", pch=21,bg=binder$myCol, main="Lat and clim SDM")
	abline(v=43.90, col="lightgrey", lty=2)
	plot(binder$Latitude, binder$V2, xlab="Latitude", ylab="Lambda", pch=21,bg=binder$myCol, main="Lat and IPM")
	abline(v=43.90, col="lightgrey", lty=2)
	plot(binder$RFavg, binder$V2, xlab="SDM suitability", ylab="Lambda", pch=21, bg=binder$myCol, main="clim SDM and IPM")
	abline(v=43.90, col="lightgrey", lty=2)
	
	
	plot(binder$Latitude, binder$RF.modprob, xlab="Latitude", ylab="Suitability", pch=21,bg=binder$myCol, main="Lat and stream SDM")
	abline(v=43.90, col="lightgrey", lty=2)
	plot(1, type="n", axes=F, xlab="", ylab="")
	plot(binder$RF.modprob, binder$V2, xlab="SDM suitability", ylab="Lambda", pch=21, bg=binder$myCol, main="stream SDM and IPM")
	abline(v=43.90, col="lightgrey", lty=2)
...
# VIOLIN PLOT 	~ site lambdas
	my_vioplot(siteLambdas$LOOK[complete.cases(siteLambdas$LOOK)],
			siteLambdas$ROCK[complete.cases(siteLambdas$ROCK)],
				...
				siteLambdas$THOMAS[complete.cases(siteLambdas$THOMAS)],
				siteLambdas$HUNTER[complete.cases(siteLambdas$HUNTER)],
				col=BarCols, names=binder$name, ylim=c(0, 4))
				abline(h=1, col="darkgrey", lty=2)
