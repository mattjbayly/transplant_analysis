...
require(fields)
	...
	tobump <- names(m.par.LOOK)

	for(w in 1:length(tobump)){
		thisRound <- tobump[w]
...
setwd(paste0(path.obj, "/Elasticity2"))
write.csv(siteLambdas, file=paste0("siteLambdas_", tobump[w], ".csv"))
setwd(path.obj)
	}
...
# load in original 
setwd(path.obj)
mainLam <- read.csv(file="siteLambdas_MoistScore0_noInt.csv")
mainLam <- mainLam[ ,c("V1","V2")]
colnames(mainLam) <- c("SiteID", "Lam")

	for(w in 1:length(tobump)){
		setwd(paste0(path.obj, "/Elasticity2"))
		temp <- read.csv(file=paste0("siteLambdas_", tobump[w], ".csv"))
		temp <- temp[ ,c("V1", "V2")]
		temp$V2 <- (temp$V2/mainLam$Lam)
		temp$V2 <- abs(temp$V2 - 1)
		colnames(temp) <- c("SiteID", tobump[w])
		mainLam <- merge(mainLam, temp, by.x="SiteID", by.y="SiteID")
		}
		

target <- c(5,4,8,1,3,2,7,6)
mainLam$target  <- target
mainLam <- mainLam[order(mainLam$target), ]
mainLam  <- subset(mainLam , select = -c(target, Surv_moist_score, Grow_moist_score,
		Repr_moist_score, Fec_moist_score) )

cols <- c("coral","coral", "coral", "coral",  "blue4", "blue4", "blue4", "blue4")
labs <- c("SiteID", "lam", "S-Int", "S-Slope", "S-Int-Site", "G-Int", "G-Slope", "G-sd", 
		"G-Int-Site", "G-Slope-Site", "R-Int", "R-Slope", "R-Int-Site", "F-Int", 
		"F-Slope", "F-Int-Site", "F-Slope-Site", "SPF", "Estb", "m-Recr", "sd-Recr")
colnames(mainLam) <- labs


setwd(path.fig)
pdf(file = "06.3.elasticity_Moistsingle.pdf", width=(8.5 - (1.25 + 0.75)), height=(6), family="Times") #  - (0.75 + 0.75)
	par(ps=10, mfrow=c(5,4), mar=c(3,3,2,0.5), mgp = c(1, 0.2, 0), oma=c(0.5,0,0,0)) # http://www.programmingr.com/content/controlling-margins-and-axes-oma-and-mgp/
	par(mfrow=c(4,2))

for(i in 1:8){

barplot(as.matrix(mainLam[i,3:ncol(mainLam)]), 
	ylim=c(min(mainLam[,3:ncol(mainLam)]), max(mainLam[,3:ncol(mainLam)])),
	col=cols[i], main="", ylab="", las = 2)
	title(mainLam[i,1], line = -1)
	mtext("Elasticity", side=2, line=2, cex=0.8)
}
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	just <- mainLam[, c(3:ncol(mainLam))]
	just_mean <- colMeans(just, na.rm = TRUE, dims = 1)
b <- barplot(just_mean, 
	ylim=c(min(mainLam[,3:ncol(mainLam)]), max(mainLam[,3:ncol(mainLam)])),
	col="lightgrey", main="", ylab="Elasticity", las = 2)

	
setwd(path.fig)	
pdf(file = "06.4.elasticity_Moist_single.pdf", width=(8.5 - (1.25 + 0.75)), height=(3), family="Times")
	par(ps=10, mar=c(3,3,2,1), mgp = c(2, 0.5, 0), oma=c(2,0,0,0)) # http://www.programmingr.com/content/controlling-margins-and-axes-oma-and-mgp/
	par(mfrow=c(1,1))

	just <- mainLam[, c(3:ncol(mainLam))]
	just_mean <- colMeans(just, na.rm = TRUE, dims = 1)
	just_min <- apply(just,2,min)
	just_max <- apply(just,2,max)

barplot(just_mean, 
	ylim=c(min(mainLam[,3:ncol(mainLam)]), max(mainLam[,3:ncol(mainLam)])),
	col="lightgrey", main="Plot-level moisture as a covariate", ylab="Elasticity", las = 2)
	segments(b,just_mean,b,just_min, lwd=1)
	segments(b,just_mean,b,just_max, lwd=1)
