###########################################################
# GROWTH FUNCTIONS FOR 2010 - 2012 DEMOGRAPHY DATA
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	pdf(file="04.2.2_Growth_Explor.pdf", width=11, height=8.5)
	...
	setwd(path.funct)
	require(RCurl)
	source("demo_04.2.2.extraplotsGrowth.R")
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	#############################################################################	
		# Start with as many fixed parameters as practically possible. 
			...
			Bbase <- gls(z1 ~ z*Region*Year, method = "REML", data = grow.plot.data)
			Bplot <- lme(z1 ~ z*Region*Year, data = grow.plot.data, random = ~1 | PlotID, method = "REML")
			Bsite <- lme(z1 ~ z*Region*Year, data = grow.plot.data, random = ~1 | SiteID, method = "REML")
			Bboth <- lme(z1 ~ z*Region*Year, data = grow.plot.data, random = ~1 | SiteID/PlotID, method = "REML")
			B_Stupidly_complex <- lme(z1 ~ z*Region*Year, data = grow.plot.data, random = ~z | SiteID/PlotID, method = "REML")
			#B_Really_Stupidly_complex <- lmer(z1 ~ z*Region*Year + (z|SiteID:PlotID) + (1|ID), data = grow.plot.data)
			AIC(Bbase, Bplot, Bsite, Bboth, B_Stupidly_complex)
	...
	setwd(path.fig); pdf(file="04.2.2_Growth_RandStructAssum.pdf", width=11, height=8.5)
	...
	for(i in 1:length(possible_rand_structs)){	
		...
		boxplot(E2 ~ Year, data = grow.plot.data, main = "Year", ylab = MyYlab) # weak/no evidence for unequal variance
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	# Switch from REML -> ML to compare fixed effects. 
		Form <- formula(z1 ~ z + Region + Year + Region*Year + Region*z + Year*z)
		M1.Full <- lme(Form, random =~ 1 | SiteID, method = "ML", data = grow.plot.data)		
		M1.A <- update(M1.Full, .~. -Year*z) # The model with year*z has only marginally signifigant p-value & aic of just 2.2 points better (should drop)
		M1.B <- update(M1.Full, .~. -Region*z)
		M1.C <- update(M1.Full, .~. -Region*Year) # drop year*region interaction
		anova(M1.Full, M1.A)
		anova(M1.Full, M1.B)
		anova(M1.Full, M1.C)
	# drop terms from above update model & try to drop additional terms
		Form2 <- formula(z1 ~ z + Region + Year + Region:z)
		M2.Full <- lme(Form2, random =~ 1 | SiteID, method = "ML", data = grow.plot.data)		
		M2.A <- update(M2.Full, .~. -Year) # 
		M2.B <- update(M2.Full, .~. -Region -Region:z) # 
		M2.C <- update(M2.Full, .~. -Region:z) # 
		M2.D <- update(M2.Full, .~. -z -Region:z) # 
		anova(M2.Full, M2.A)
		anova(M2.Full, M2.B)
		anova(M2.Full, M2.C)
		anova(M2.Full, M2.D)
		# looks like this is the best we can do to simplify the model 
	
	# 5.10.8 Step 9 of the Protocol: Refit with REML and Validate the Model
		M3 <- lme(z1 ~ z + Region + Year + Region*z, random =~1|SiteID,
				method = "REML", data = grow.plot.data)
		summary(M3); methods(class = "lme")
	...
	# 5.10.10 pg. 139 
		...
		library(mgcv)
		M6 <- gamm(z1 ~ Region + s(z),
			random = list(SiteID = ~ 1), data = grow.plot.data)
		anova(M6$gam)
...
##################################################################################
	...
	# MODEL SELECTION 
		...
		M2A.noYr <- M2.A; M2B.noReg <- M2.B; M2C.noZReg <- M2.C; M2D.noZ <- M2.D
		out_put <- mod.sel(M2.Full, M2A.noYr, M2B.noReg, M2C.noZReg, M2D.noZ)
		modList <- c("M2.Full", "M2A.noYr", "M2B.noReg", "M2C.noZReg", "M2D.noZ")
		out_put
	...
	# MAKE CLEAN AICc TABLE & SAVE
		...
		sel.table <- as.data.frame(out_put)[6:10] 			
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	library(lmerTest)
	# refit with lmer
	M2.A <- lmer(z1 ~ z + Region + Region:z + (1|SiteID), data = grow.plot.data)		
...
# resample our data with 1000 replicates													
	...
	myboot <- function(i) {
		object <- try(lmer(z1 ~ z + Region + Region:z + (1|SiteID), na.action=na.omit, data = bigdata,
			subset = Replicate == i, start = list(fixef = f, theta = r)), silent = TRUE)
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	# Make plots of results 
		...
		head(plotdat)
		setwd(path.fig)
		pdf(file="04.2.2_Growth_ResponsePlots.pdf", width=11, height=8.5)
		...
		# convert regions down to just three levels N, C & S and make year a factor variable 
		library(plyr)
