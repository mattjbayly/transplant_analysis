#title: "EXPLORATORY REGRESSION OF PLOT LEVEL ENVIRONMENTAL VARIABLES WITH VITAL RATES"
#author: "Matthew Bayly"
#modified: "Saturday, January 17, 2015"

# PURPOSE OF SCRIPT: 
# were vital rates strongly dependent on any of the measured
# plot-level variables? Were the functional forms of these 
# relationships also similar across sites?
# Rational: If the effects of site-level climatic differences
#on vital rates were subtle in comparison to plot level
# differences, then it will be necessary to incorporate 
# these plot-level variables into the final models.
 
# Goal: to produce final figures similar to FigureS 3 & 4 in:
	# Diez, JM. Giladi, I., Warren, R., Rulliam, HR. (2014): Probabilistic 
	# and spatially variable niches inferred from demography.
	# Journal of Ecology, 102, 544–554.
	
# INDEX: 
	# 0. load libraries & set directories to computer
	# 1. Load revised plant data frame & plot-level data (check details)
	...
	# 3. Run a quick PCA  for plot level variables
		# 3.1 Temporarily include PCA1, PCA2, PCA3 as candidate predictor variables
		# * Do the first three PCA axis make sense (can they be described as something)? 
	# 4. For each vital rate explore variable importance among candidate 
		# predictor variables (Code: Niklaus E. Zimmermann)
		# 4.1 variable importance function 
		# 4.2 build scatter plots
	# 5. Run functional forms for each site of 2 - 3 key predictors. 
	# - mem, for site with variable slope? 
	# - CI for coefficents of enviro varibles 
	# - 
	# 5. Save output, plots & revised data frame
