...
setwd(paste0(path.obj, "/Elasticity"))
...
# load in original 
setwd(path.obj)
mainLam <- read.csv(file="siteLambdas.csv")
...
mainLam  <- subset(mainLam , select = -c(target) )
...
setwd(path.fig)
pdf(file = "06.1.elasticity_MEM_single.pdf", width=(8.5 - (1.25 + 0.75)), height=(6), family="Times") #  - (0.75 + 0.75)
	par(ps=10, mfrow=c(5,4), mar=c(3,3,2,0.5), mgp = c(1, 0.5, 0), oma=c(0.5,0,0,0)) # http://www.programmingr.com/content/controlling-margins-and-axes-oma-and-mgp/
	par(mfrow=c(4,2))
...
barplot(as.matrix(mainLam[i,3:ncol(mainLam)]), 
	...
	title(mainLam[i,1], line = -2)
	mtext("Elasticity", side=2, line=2, cex=1)
}
...
setwd(path.fig)	
pdf(file = "06.2.elasticity_MEM_single.pdf", width=(8.5 - (1.25 + 0.75)), height=(3), family="Times")
