...
# scale residuals so kernel variance equals
# the estimate of growth variance
...
px <- seq(-4*sdhat,4*sdhat,length=500)
#matplot(px,cbind(kfun(px),dnorm(px,mean=0,sd=5)),type="l", ylim=c(0,1.25*max(kfun(px))),
...
#dev.copy2eps(file="~/repos/ipm_book/HowTo/kernelExample.eps")
