...
while(yr < n.yrs & length(z) < 5000) {
    ...
    repr[i.subset] <- rbinom(n = length(i.subset), prob = pb_z(z[i.subset], m.par), size=1)
    ...
    recr[i.subset] <- rbinom(n = length(i.subset), prob=pr_z(m.par), size=1)
    ...
    z <- c(z1.rec[which(surv == 1 & repr == 1 & osex==1 & recr == 1)], z1[which(surv == 1)])
