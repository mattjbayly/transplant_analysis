###########################################################
# FLOWERING FUNCTION FOR 2010 - 2012 DEMOGRAPHY DATA
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	#############################################################################	
			...
			pdf(file="04.2.3_Flower_Explor.pdf", width=11, height=8.5)
		...
		# additional plots in another script to save space on this sheet & make for efficent coding 
			setwd(path.funct)
			source("demo_04.2.3.extraplotsRepr.R")
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	setwd(path.fig)
	pdf(file="04.2.3_Flower Random Effect Structure.pdf", width=11, height=8.5)
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	# Largest possible full model:
		#"Repr ~ z + Year + Region + z*Year + z*Region + z*Year*Region"
	...
	#mod1 <- glmer(Repr ~ z + Year + Region + z*Year + z*Region + z*Year*Region + (1|SiteID) + (1|PlotID) + (1|SiteID:PlotID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod2 <- glmer(Repr ~ z + I(z^2) + Year + Region + z*Year + z*Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod3 <- glmer(Repr ~ z + I(z^2) + Year + Region + z*Year + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod4 <- glmer(Repr ~ z + I(z^2) + Year + Region + z*Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod5 <- glmer(Repr ~ z + I(z^2) + Year + Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod6 <- glmer(Repr ~ z + I(z^2) + Region + z*Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod7 <- glmer(Repr ~ z + I(z^2) + Year + z*Year + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod8 <- glmer(Repr ~ z + I(z^2) + Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod9 <- glmer(Repr ~ z + I(z^2) + Year + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod10 <- glmer(Repr ~ z + I(z^2) + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod2linear <- glmer(Repr ~ z + Year + Region + z*Year + z*Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod3linear <- glmer(Repr ~ z + Year + Region + z*Year + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod4linear <- glmer(Repr ~ z + Year + Region + z*Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod5linear <- glmer(Repr ~ z + Year + Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod6linear <- glmer(Repr ~ z + Region + z*Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod7linear <- glmer(Repr ~ z + Year + z*Year + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod8linear <- glmer(Repr ~ z + Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod9linear <- glmer(Repr ~ z + Year + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	mod10linear <- glmer(Repr ~ z + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
	...
	# MODEL SELECTION 
	#################
		out_put <- mod.sel(mod2,mod3,mod4,mod5,mod6,mod7,mod8,mod9,mod10,mod2linear,mod3linear,mod4linear,mod5linear,mod6linear,mod7linear,mod8linear,mod9linear,mod10linear)
		modList <- c("mod2","mod3","mod4","mod5","mod6","mod7","mod8","mod9","mod10", "mod2linear","mod3linear","mod4linear","mod5linear","mod6linear","mod7linear","mod8linear","mod9linear","mod10linear")
		out_put
	# absolutely include quadratic term 
	...
	# MAKE PREDICTIONS	
		...
		# Check for overdispersion 
			...
			overdisp.glmer(mod3) # seems ok, a bit under-dispersed 
			overdisp.glmer(mod7) # seems ok, a bit under-dispersed 
		...
		mod3_glmmML <-glmmML(formula=Repr ~ z + I(z^2) + Year + z*Year + Region,
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	# QUANTILE-QUANTILE PLOTS (note that the functions "fitted.glmmML" and "res.glmmML" are requiured here, loaded in line above)	
		...
		Model_Best <- mod3_glmmML # best model from steps above
		...
		Models<-apply(Sims,MARGIN=2,FUN=function(X){return(list(X,glmmML(formula=Repr ~ z + Year + z * Year + Region,cluster=SiteID,data=Repr.plot.data,family=binomial)))})
		...
		dev.off()
		setwd(path.fig)# close existing plotting frame, switch directories and open a new file for saving plots  
		pdf(file="04.2.3_Flower_model_adequacy.pdf", width=11, height=8.5)
...
# Comment: "Things seem OK from the quantile-quantile plot, but 
		...
		mod3_glmmML_NoQuadratic <-glmmML(formula=Repr ~ z + Year + z*Year + Region, cluster=SiteID,data=Repr.plot.data,family=binomial)
		mod3_glmmML_Quadratic <-glmmML(formula=Repr ~ z + I(z^2) + Year + z*Year + Region, cluster=SiteID,data=Repr.plot.data,family=binomial)
		summary(mod3_glmmML_Quadratic)
		AIC(mod3_glmmML_NoQuadratic) # lower AIC from model with quadratic term ... 
		# but maybe this is just a regional effect 
		Quadratic2NoRegion <-glmmML(formula=Repr ~ z + I(z^2) + Year + z:Year, cluster=SiteID,data=Repr.plot.data,family=binomial)
		AIC(Quadratic2NoRegion)
		# shoot... ok will put the quadratic term in 
		# this also makes sense ecologically

# Add quadratic term into mod3 
		mod3 <- glmer(Repr ~ z + I(z^2) + Year + z*Year + Region + (1|SiteID), na.action=na.omit, data=Repr.plot.data, family = binomial)
		mod3_glmmML <-glmmML(formula=Repr ~ z + I(z^2) + Year + z*Year + Region , cluster=SiteID,data=Repr.plot.data,family=binomial)
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	# difference across models so far 
		...
		se <- sqrt(diag(vcov(mod3))); (tab <- cbind(Est = fixef(mod3), LL = fixef(mod3) - 1.96 * se, UL = fixef(mod3) + 1.96 * se))
		# basic glm below 
	...
	# Steps: refit model to sample data
		...
		f <- fixef(mod3) # store original parameters 
		r <- getME(mod3, "theta") 
		cl <- makeCluster(4)
		clusterExport(cl, c("bigdata", "f", "r"))
		clusterEvalQ(cl, require(lme4))
				myboot <- function(i) {
					object <- try(glmer(Repr ~ z + I(z^2) + Year + z*Year + Region + (1|SiteID), na.action=na.omit, data = bigdata, subset = Replicate == i, family = binomial,
...
## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		save(mod3, file = "Reprmod3.rda")
		
		# save boot replicate datasets
		write.csv(bigdata,"BootReplicateDataSets.csv", row.names = T) 
	...
	# Make plots of results 
		...
		# calculate predicted probabilities and store in a list
			pp <- lapply(jvalues, function(j) {
				tmpdat$z <- j
				predict(mod3, newdata = tmpdat, type = "response")
			})
		...
		head(plotdat)
		setwd(path.fig)
		pdf(file="04.2.3_Flower_ResponsePlots.pdf", width=11, height=8.5)
