...
# Year Specific
	...
	####################################
		...
		legend('topright', legend = levels(dfclean$Year), col = 1:2, cex = 0.8, bg="white", pch = 19)
...
# REGION SPECIFIC 	
			...
			abline(0, 1, lty=2) # line of zero growth (not positive or negative), useful for negative growth at large size classes. 
	####################################
		secondyr <- dfclean[which(dfclean$Region == "N"), ]
			z <- secondyr$z # stand along object 
			grow.plot.data <- subset(secondyr, !is.na(z1))
			# Growth regression, simple linear - will do mixed model  with regions later
			mod.grow <- lm(z1 ~ z, data = grow.plot.data)
			...
			abline(mod.grow, col=2, lwd=2)
			abline(0, 1, lty=2) # line of zero growth (not positive or negative), useful for negative growth at large size classes. 
		text(1, 8, paste0("Int ", as.character(round(coef(mod.grow), 2))[1]), col=2)
		text(1, 7, paste0("Slope ", as.character(round(coef(mod.grow), 2))[2]), col=2)
	####################################
		secondyr <- dfclean[which(dfclean$Region == "S"), ]
			z <- secondyr$z # stand along object 
			grow.plot.data <- subset(secondyr, !is.na(z1))
			# Growth regression, simple linear - will do mixed model  with regions later
			mod.grow <- lm(z1 ~ z, data = grow.plot.data)
				points(z1 ~ z, data = grow.plot.data, pch = 19, cex = 0.7, col=3)
			# fitted line from growth regression 
			abline(mod.grow, col=3, lwd=2)
			abline(0, 1, lty=2) # line of zero growth (not positive or negative), useful for negative growth at large size classes. 
		text(3, 8, paste0("Int ", as.character(round(coef(mod.grow), 2))[1]), col=3)
		text(3, 7, paste0("Slope ", as.character(round(coef(mod.grow), 2))[2]), col=3)
		legend('topright', legend = levels(dfclean$Region), col = 1:3, cex = 0.8, bg="white", pch = 19)
	...
	for(i in 1:length(sites)){
		firstyr <- dfclean[which(dfclean$SiteID == sites[i]), ]
			...
			mod.grow <- lm(z1 ~ z, data = grow.plot.data)
				plot(z1 ~ z,
					 data = grow.plot.data,
					 xlim = plot.range, ylim = plot.range, pch = 19, cex = 0.7, col="darkgrey",
					 xlab = expression("Initial size, "*italic(z)),
					 ylab = expression("Final size, "*italic(z)*"'"),
					 main=sites[i])
			######
